...
需求描述见 `docs/requirements.md`，本文件只记踩过的坑。
...
- 装真机固定走 `just install`（已封装 opt-level 3 构建、备份、原子换二进制、换代顺序、前后 pid 核对）；`pm3 service install` 用 `current_exe()` 渲染 unit → MUST NOT 在仓库目录执行（会把 plist 钉在 `target/release/pm3`，一次 `cargo clean` 就起不来），先把二进制 `cp` 到最终位置，再用**那个**二进制执行 install
- 任何 pm3 CLI 命令都会经 `ensure_daemon_running` 自动拉起一个**非 launchd 托管**的 daemon：它扛不住 `launchctl unload`，且会抢赢 socket 竞争让 launchd 那份直接退出（症状：`launchctl list` 的 PID 列是 `-`，job 已载入但 launchd 未监管、KeepAlive 形同失效）→ 换代顺序 MUST 是 `service uninstall` → `pm3 kill` → 等 `pgrep -f "<bin> daemon"` 归零 → `service install --force`；install 后 MUST 等「launchd 报的 pid == `pm3.pid` 内容」再跑任何 CLI 命令，否则又会拉起竞争者。已出现未监管态时先 `pm3 kill` 停掉自启实例，再 `launchctl kickstart gui/$(id -u)/<label>` 交回 launchd
...
- 定位缺口：`just cov` 失败却一行文件明细都没打 = 缺的是 **region**（lcov 不含 region 数据，`findFilesBelowFullCoverage` 自然无输出）→ MUST 紧跟在一次 `just cov --fresh` 之后（中途不插其他 cargo 命令）跑 `cargo +nightly llvm-cov report --release --summary-only | awk 'NR>2 && $3+0>0'` 找文件，再 `--show-missing-lines`：无输出且 lines 也缺 → 缺口在 bin 副本（lib+bin 双编译，region 按实例化计数，补 e2e 走真实 binary 或让分支只存在于一处）；无输出而 lines 100% → 缺的是 `?`/短路的纯 region，重点怀疑新加的 `?`。查完回到 `--fresh`
...
- macOS 上 `sandbox-exec` 的 `subpath` 只认真实路径，`/var/...` 这类符号链接不匹配 → spawn 前必须 canonicalize `cwd` 与 `writable_roots`；`materialise_workspace` 里展开 `${PM3_SVC_CWD}` MUST 排在 `spec.cwd = real_path(...)` 之后，提前替换会把未 canonicalize 的 cwd 写进 args 正好复现该陷阱（回归测试：`adapters/src/tests/workspace_tests.rs::a_placeholder_expands_to_the_real_path_not_the_symlink` 与 `frameworks/tests/sandbox_isolation.rs::a_confined_app_can_write_through_the_cwd_placeholder`）
...
- cron 到点只调 `restart_app`、**不新增状态**；判定「注册时是否 spawn」的分支 MUST 落在注册路径（`start_apps` 的 `StartMode::Register`）而非执行路径，否则 cron 触发时也会被跳过、任务永不运行
...
- **Rust 生态没有任何 cron 库支持 OpenBSD 风格的随机 `~`**（croner/cron/cronexpr/jiff-cron/cron_tab 全无，只有 cronexpr 支持 Jenkins 的固定哈希 `H`）→ `adapters/src/schedule/random_expand.rs` 自己展开成具体数字再交 croner；每次 `arm_timer` 重新展开，副产品是「早晚各一次」写成 `25~35 9,18 * * *` 时两次会落在不同分钟
...
- CLI 早期校验 MUST 用 `pm3.search_path` 而非 `std::env::var("PATH")`
