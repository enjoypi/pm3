...
- [ ] install.sh 真机验收：v1.11.2 已推送触发 release.yml（含 .sha256、config.yaml、Windows 产物与 windows e2e job；v1.11.1 缺 zip 应删除）；确认 release 产出后，`HOME=$(mktemp -d)` 跑 install.sh 验首装路径，再跑一次验升级路径（匿名下载仍需仓库转 public）：v1.11.1 已推送触发 release.yml（含 .sha256、config.yaml、Windows 产物与 windows e2e job）；确认 release 产出后，`HOME=$(mktemp -d)` 跑 install.sh 验首装路径，再跑一次验升级路径（匿名下载仍需仓库转 public）
