...
fn a_service_that_was_never_installed_reads_as_not_installed() {
    let home = home();
    let queried = pm3(&home, &["startup", "--status"]);
...
fn a_dry_run_install_prints_the_unit_without_touching_the_host() {
    let home = home();
    let planned = pm3(&home, &["startup", "--dry-run"]);
    ...
    assert!(
        stdout_of(&pm3(&home, &["startup", "--status"])).contains("not installed"),
...
#[test]
fn a_dry_run_unstartup_prints_the_removal_plan() {
    let home = home();
    let planned = pm3(&home, &["unstartup", "--dry-run"]);
    assert!(planned.status.success(), "{}", stdout_of(&planned));
}
