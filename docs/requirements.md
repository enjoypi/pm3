...
Credentials may also be kept encrypted instead of in plain text: a `<service-name>.enc.yaml` file beside the configuration, encrypted with sops. Whenever pm3 reads that service's environment — at start, at restart, and when one daemon hands over to the next — the file is decrypted, and what it holds replaces the plaintext file entirely. It is only opened when the configuration names an identity to decrypt with; without one pm3 falls back to the plaintext file, says in the log that it left the encrypted one closed, and marks the service, rather than quietly running it on credentials nobody meant to use. A decrypted value reaches the service exactly as it was encrypted — quotes, backslashes and trailing spaces are all part of the secret — with the single exception of `$HOME`, which is expanded as above. Because such a file belongs to the service it is named after, a service name may not end in `.enc`; deleting a service deletes its encrypted file too.

`pm3 describe` always names where a service's environment came from and how much of it there is: `plain (2 values)`, `encrypted (3 values)`, or `encrypted, not opened (0 values)`. The count is what the operator declared; the `HOME` pm3 hands out on its own is not counted. `pm3 list` stays exactly as it is while all is well, and marks only the rows of services running without the credentials they declare, so one screen still answers "is anything wrong".

Decryption depends on a program outside pm3, so failing to decrypt is treated differently from a broken file: a daemon replacing another that cannot decrypt stops without taking over at all, leaves the running services untouched, and says why. A decryptor that was merely slow or briefly unavailable must never cost a healthy service its life. Under a service manager this repairs itself — pm3 is started again, and the takeover happens on the first attempt where decryption works, with the original processes reclaimed rather than restarted.
...
A service can also declare a liveness endpoint (`liveness_tcp: "127.0.0.1:8080"`). pm3 then keeps connecting to it while the service is online, and once the connection has failed the configured number of times in a row it takes the service down — a process that is still running but no longer answering is exactly what an exit-based restart cannot catch. The restart that follows answers to the breaker like a crash does, so a service that never recovers ends up errored rather than cycling. Only tcp endpoints are accepted: a probe that repeats forever must not fork. Startup is left to the ready probe; liveness only watches a service that already reached online.

Memory leaks also have a backstop: give a service a memory limit (`--max-memory-restart 300M`), and pm3 periodically samples its resident memory and restarts it when it exceeds the limit. Services without a configured limit are unaffected and are not sampled. A restart pm3 decided on itself answers to the same breaker as a crash does — it backs off and it gives up — so a service that leaks its way past the limit again and again ends up errored instead of cycling forever. A restart you asked for is never held back that way.
