...
#[test]
fn redact_url_no_scheme_returns_unchanged() {
    assert_eq!(redact_url("weird-no-scheme-url"), "weird-no-scheme-url");
}

#[test]
fn redact_url_credentials_without_path() {
    assert_eq!(
        redact_url("postgres://user:pass@host"),
        "postgres://***@host"
    );
}

#[test]
fn redact_url_redacts_password_query_parameter() {
    assert_eq!(
        redact_url("postgres://appuser@db:5432/app?password=hunter2"),
        "postgres://***@db:5432/app?password=***"
    );
}

#[test]
fn redact_url_redacts_password_query_parameter_without_userinfo() {
    assert_eq!(
        redact_url("postgres://db:5432/app?password=hunter2"),
        "postgres://db:5432/app?password=***"
    );
}

#[test]
fn redact_url_at_sign_inside_query_is_not_treated_as_credentials() {
    assert_eq!(
        redact_url("postgres://db/app?password=a@b"),
        "postgres://db/app?password=***"
    );
}

#[test]
fn redact_url_redacts_api_key_query_parameter() {
    assert_eq!(
        redact_url("postgres://db/app?api_key=hunter2"),
        "postgres://db/app?api_key=***"
    );
}

#[test]
fn redact_url_redacts_auth_and_credential_query_parameters() {
    assert_eq!(
        redact_url("mysql://db/app?authToken=a&credentials=b"),
        "mysql://db/app?authToken=***&credentials=***"
    );
}

#[test]
fn redact_url_keeps_non_secret_query_parameters() {
    assert_eq!(
        redact_url("sqlite://data.db?mode=rwc"),
        "sqlite://data.db?mode=rwc"
    );
}

#[test]
fn redact_url_redacts_only_the_secret_parameter_among_several() {
    assert_eq!(
        redact_url("mysql://db/app?charset=utf8&PASSWORD=hunter2&ssl-mode=required"),
        "mysql://db/app?charset=utf8&PASSWORD=***&ssl-mode=required"
    );
}

#[test]
fn redact_url_keeps_valueless_query_parameter() {
    assert_eq!(
        redact_url("sqlite://data.db?immutable"),
        "sqlite://data.db?immutable"
    );
}
...
#[test]
fn load_config_file_exists() {
    let dir = tempfile::tempdir().expect("create temp dir");
    let path = dir.path().join("test-config.yaml");
    std::fs::write(&path, "server:\n  host: localhost\n").expect("write temp file");
    let result = load_config(path.to_str().expect("valid path"));
    assert!(result.is_ok());
    assert!(result.expect("should load").contains("localhost"));
}
