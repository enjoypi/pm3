...
- `definesImplementationItems`'s regex MUST accept **every** visibility form (`pub`, `pub(crate)`, `pub(super)`, `pub(in path)`): a form it does not match silently exempts that file from the "production file missing from lcov" self-check, so a real function can drop out of the gate unnoticed (`adapters/src/apps_file/roots.rs` did)
